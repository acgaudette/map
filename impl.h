...
{
	return mulff((ff) { node->x, node->y }, GRID_SCALE);
}
...
static ff node_ext(const node *node)
{
	...
	return mulff(result, (SCALE * grow));
...
static int circle_circle(struct man a, struct man b)
{
	...
	float dist = magff(subff(a.pos, b.pos));
...
static void update()
{
	...
	if (dirty.cam) {
		...
		cam.pos = (ff) {
			lerpf(ext_x.x, ext_x.y, .5),
			lerpf(ext_y.x, ext_y.y, .5),
		};
...
static void sprite_draw_edge(struct sprite sprite, struct txt_buf *txt)
{
	...
	quad.model = m4_model_aniso(
		v3_add(sprite.pos, qt_app(sprite.rot, v3_mul((v3) { -.5, (-.5 + thick), 0 }, sprite.scale))),
		sprite.rot,
...
static void render(txt txt)
{
	...
	VBUF_FOREACH(nodes, node) {
		...
		struct block_ctx ctx = block_prepare(
			(struct block) {
				...
				.scale = scale,
				.pos = (v3) { root.x, root.y, 0 },
				.rot = qt_axis_angle(V3_FWD, (angle * .05)),
				.anch = (ff) { 0, 0 },
				.justify = JUST_CENTER,
				.spacing = 1.f,
				.line_height = 1.f,
		...
		if (asp < MAX_CIRCLE_ASP && asp > 1.f / MAX_CIRCLE_ASP) {
			...
			for (u16 i = 0; i < n; ++i) {
				...
				const ff pos = addff(root, mulff((ff) { sinf(a), cosf(a) }, r));
	...
	VBUF_FOREACH(edges, edge) {
		...
		const ff diff = subff(b_pos, a_pos);
		const ff dir = normff(diff);
		...
		for (u16 i = 0 + 1; i < n - 1; ++i) {
			...
			const ff pos = mixff(a_pos, b_pos, x);
